...
if ($user_number == '') {
    ...
    $welcome_stmt = $conn->prepare("SELECT 
    b.BOARD_ID, 
    b.USER_NUMBER, 
    b.BOARD_TITLE, 
    b.BOARD_CONTENT, 
...
WHERE  
    b.BOARD_CATEGORY = '가입인사'");
    // $welcome_stmt->bind_param("i", $user_number); 

    if ($welcome_stmt->execute()) { // 게시글 가입인사 select 문 성공
        $result = $welcome_stmt->get_result();
        if ($result->num_rows > 0) { // 가입인사 게시글 존재 
