...
if ($user_number == '') {
    ...
    $recommend_stmt = $conn->prepare("SELECT 
    b.BOARD_ID, 
    b.USER_NUMBER, 
    b.BOARD_TITLE, 
    b.BOARD_CONTENT, 
...
WHERE  
    b.BOARD_CATEGORY = '책 추천'");
    // $recommend_stmt->bind_param("i", $user_number);

    if ($recommend_stmt->execute()) { // 게시글 전체 select 문 성공
