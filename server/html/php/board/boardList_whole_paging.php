...
if ($user_number == '') { 
    ...
    $whole_stmt = $conn -> prepare("SELECT 
    BOARD_ID, 
    USER_NUMBER, 
    BOARD_TITLE, 
    BOARD_CONTENT, 
...
FROM (
    SELECT 
        ...
        COALESCE(bl.LIKE_STATUS, 0) AS like_or_not,
        ROW_NUMBER() OVER (ORDER BY b.CREATE_TIME DESC) AS rn
    FROM 
        BOARD b
    LEFT JOIN 
    ...
    LEFT JOIN 
        (SELECT BOARD_ID, LIKE_STATUS 
         FROM BOARD_LIKE 
         WHERE USER_NUMBER = ?) bl ON b.BOARD_ID = bl.BOARD_ID
       
) subquery
WHERE rn <= 5
ORDER BY rn DESC"); 
    $whole_stmt-> bind_param("i", $user_number); 
    
    if($whole_stmt->execute()) { // 게시글 전체 select 문 성공
        $result = $whole_stmt -> get_result(); 
